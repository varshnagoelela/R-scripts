...
#standard variabeles:
ns='vetteKip' #general name of the study
...
for (fn in infiles) {
  ...
  # read table fn
  samplePath = paste(indir, fn, sep = "")
  sample <- read.table(samplePath,
                            header=T,  
                            stringsAsFactors = F,
                            sep='\t',
                            quote="")
...
file.nm = paste(indir , paste(ns, 'cutoff', x=dateTime(),'txt', sep="."), sep="")
write.table(cutoffM,
            file = file.nm,
            quote= FALSE,
            sep='\t',
...
file.nm = paste(indir, paste(ns, 'cutoff', x=dateTime(),'xls', sep=","), sep="")
write.table(cutoffM,
            file = file.nm,
            quote= FALSE,
            sep='\t',
