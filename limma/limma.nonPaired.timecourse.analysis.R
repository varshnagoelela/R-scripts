#/**
#* ##############################################################
#* Limma analysis - paired samples with time course
...
contrast.fit <- "Dif6hr = (M1.360-M1.0)-(H1.360-H1.0)"

if (require(limma)) {
  #create design and apply first first fit and eBayes
  design <- model.matrix(~0+groupTime)
  colnames(design) <- lev
  rownames(design) <- description[,hdr]
  fit <- lmFit(data, design) 
  #compare 2 groups 
  cont.dif <- makeContrasts(
    Dif6hr = (M1.360-M1.0)-(H1.360-H1.0),
    levels=design)
...
results_fn = paste(sub("input", "output", indir),paste('LIMMA', ns, contrast.fit, x=dateTime(), 'txt', sep='.'), sep="" )
write.table(results, 
            file      = results_fn,
            row.names = FALSE, 
            quote     = FALSE, 
